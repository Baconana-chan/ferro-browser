...
impl<T: ClipboardProvider> TextInput<T> {
    ...
    /// Begin a new mouse selection at the given grapheme cluster index.
    /// This sets both the selection origin and edit point to the same position.
    pub fn begin_selection_at_index(&mut self, index: usize) {
        let byte_offset = self.lines[self.edit_point.line]
            .str()
            .graphemes(true)
            .take(index)
    ...
    pub fn extend_selection_to_index(&mut self, index: usize) {
        ...
        // Determine selection direction based on origin and new edit point
        if let Some(origin) = self.selection_origin {
    ...
    /// Uses Unicode word boundaries to find word boundaries.
    pub fn select_word_at_index(&mut self, index: usize) {
        let line = &self.lines[self.edit_point.line];
        let line_str = line.str();
        ...
        // Find word boundaries using unicode_segmentation
        let mut word_start = 0;
        let mut word_end = line_str.len();
        let mut current_pos = 0;
        
        for word in line_str.split_word_bounds() {
            let word_len = word.len();
            if current_pos <= byte_offset && byte_offset < current_pos + word_len {
        ...
        // Set selection from word start to word end
        let start_point = TextPoint {
            line: self.edit_point.line,
            index: UTF8Bytes(word_start),
        };
        let end_point = TextPoint {
            line: self.edit_point.line,
            index: UTF8Bytes(word_end),
        };
    ...
    /// Select the entire current line (for triple-click).
    pub fn select_line_at_index(&mut self, _index: usize) {
        let line = &self.lines[self.edit_point.line];
        let line_len = line.len_utf8();
    ...
    /// Check if the current selection is collapsed (caret only, no actual selection range).
    pub fn selection_is_collapsed(&self) -> bool {
        match self.selection_origin {
            None => true,
            Some(origin) => origin == self.edit_point,
        }
    }
