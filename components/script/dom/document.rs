...
impl Document {
    ...
    fn update_visibility_state(&self, visibility_state: DocumentVisibilityState, can_gc: CanGc) {
        ...
        // Notify focused text controls about visibility change to pause/resume caret blinking.
        // This saves CPU when the page is in background.
        if let Some(focused) = self.get_focused_element() {
            if let Some(input) = focused.downcast::<HTMLInputElement>() {
                input.handle_visibility_change();
            } else if let Some(textarea) = focused.downcast::<HTMLTextAreaElement>() {
                textarea.handle_visibility_change();
            }
        }
