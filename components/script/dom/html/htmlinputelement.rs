...
pub(crate) struct HTMLInputElement {
    ...
    /// Whether the caret is currently visible (for blinking animation).
    /// The caret blinks at ~530ms intervals when the element is focused.
    caret_visible: Cell<bool>,
    /// Timer key for the caret blink animation. None when not focused.
    caret_blink_timer_key: Cell<Option<i32>>,
    /// Whether the user is currently selecting text with the mouse (mouse button is down).
    is_mouse_selecting: Cell<bool>,
}
...
impl HTMLInputElement {
    fn new_inherited(
        ...
        HTMLInputElement {
            ...
            caret_blink_timer_key: Cell::new(None),
            is_mouse_selecting: Cell::new(false),
        }
    }
    ...
    fn handle_key_reaction(&self, action: KeyReaction, event: &Event, can_gc: CanGc) {
        match action {
            ...
            KeyReaction::DispatchInput(text, is_composing, input_type) => {
                ...
                // Reset caret blink so it stays visible after typing
                self.reset_caret_blink();
            },
            KeyReaction::RedrawSelection => {
                self.upcast::<Node>().dirty(NodeDamage::Other);
                event.mark_as_handled();
                // Reset caret blink when selection changes
                self.reset_caret_blink();
            },
            KeyReaction::Nothing => (),
        }
...
impl<'dom> LayoutHTMLInputElementHelpers<'dom> for LayoutDom<'dom, HTMLInputElement> {
    ...
    fn selection_for_layout(self) -> Option<Range<usize>> {
        ...
        let range = UTF8Bytes::unwrap_range(sorted_selection_offsets_range.clone());

        // If there's a non-collapsed selection (actual highlighted text), always show it.
        // If there's only a caret (collapsed selection) and caret is not visible due to blinking,
        // don't show the caret.
        let is_caret_only = range.start == range.end;
        if is_caret_only && !self.caret_visible_for_layout() {
            return None;
        }
...
impl HTMLInputElement {
    ...
    /// Standard caret blink interval in milliseconds.
    /// This matches the typical OS default (Windows: 530ms, macOS: 567ms).
    const CARET_BLINK_INTERVAL_MS: i64 = 530;

    /// Check if caret blinking should be disabled.
    /// Returns true if:
    /// - Document is hidden (page not visible / tab in background)
    /// - User prefers reduced motion (accessibility)
    fn should_disable_caret_blink(&self) -> bool {
        let document = self.owner_document();

        // Check if document is hidden (tab in background)
        // When hidden, we save CPU by not blinking
        if document.Hidden() {
            return true;
        }

        // TODO: Check prefers-reduced-motion media query when available
        // Some users/OS settings disable animations for accessibility.
        // For now, this is a placeholder for future implementation:
        // if document.window().matches_media_query("(prefers-reduced-motion: reduce)") {
        //     return true;
        // }
    ...
    /// Start the caret blink timer when the input gains focus.
    fn start_caret_blink_timer(&self) {
        // Only start timer for textual inputs
        if !self.input_type().is_textual_or_password() {
            return;
        }

        // Cancel any existing timer first
        self.stop_caret_blink_timer();

        // Make caret visible immediately when focused
        self.caret_visible.set(true);

        // Don't start blinking if disabled (hidden page, reduced motion, etc.)
        if self.should_disable_caret_blink() {
            return;
        }
        ...
        let timer_key = global.run_steps_after_a_timeout(
            DOMString::from("caret-blink"),
            Self::CARET_BLINK_INTERVAL_MS,
            move |global, can_gc| {
                let input = trusted.root();
                input.handle_caret_blink_tick(global, can_gc);
            },
        );

        self.caret_blink_timer_key.set(Some(timer_key));
    }

    /// Stop the caret blink timer when the input loses focus.
    fn stop_caret_blink_timer(&self) {
        if let Some(timer_key) = self.caret_blink_timer_key.take() {
            let global = self.owner_global();
            global.clear_timeout_or_interval(timer_key);
        }
        // Reset caret visibility
        self.caret_visible.set(true);
    }

    /// Handle each tick of the caret blink timer.
    fn handle_caret_blink_tick(&self, _global: &GlobalScope, _can_gc: CanGc) {
        // Only continue blinking if still focused
        if !self.upcast::<Element>().focus_state() {
            self.stop_caret_blink_timer();
            return;
        }

        // Stop blinking if page becomes hidden or reduced motion is enabled
        if self.should_disable_caret_blink() {
            // Keep caret visible (static) but don't schedule next blink
            self.caret_visible.set(true);
            self.caret_blink_timer_key.set(None);
            return;
        }
        ...
        // Mark node as dirty to trigger repaint.
        // TODO(a11y): Update caret position in accessibility tree when a11y is implemented.
        // TODO(perf): Consider using more targeted damage (caret rect only) instead of
        // full NodeDamage::Other to avoid repainting the entire input on each blink.
        // This would require tracking caret bounds and using a specialized damage type.
        self.upcast::<Node>().dirty(NodeDamage::Other);
    ...
    /// Reset caret visibility to visible and restart the blink timer.
    /// Called when the user types or moves the caret.
    pub(crate) fn reset_caret_blink(&self) {
        if self.upcast::<Element>().focus_state() && self.input_type().is_textual_or_password() {
            self.caret_visible.set(true);
            // Restart the timer so caret stays visible for full interval after input
            self.start_caret_blink_timer();
        }
    }

    /// Called when document visibility changes (visibilitychange event).
    /// Pauses blinking when page is hidden, resumes when visible.
    pub(crate) fn handle_visibility_change(&self) {
        if !self.upcast::<Element>().focus_state() || !self.input_type().is_textual_or_password() {
            return;
        }

        if self.owner_document().Hidden() {
            // Page is now hidden - stop the timer but keep caret visible (frozen)
            if let Some(timer_key) = self.caret_blink_timer_key.take() {
                let global = self.owner_global();
                global.clear_timeout_or_interval(timer_key);
            }
            ...
            // Page is now visible - restart blinking
            self.start_caret_blink_timer();
        }
    }
}
...
impl VirtualMethods for HTMLInputElement {
    ...
    fn handle_event(&self, event: &Event, can_gc: CanGc) {
                ...
                if let Some(mouse_event) = event.downcast::<MouseEvent>() {
                    if let Some(point_in_target) = mouse_event.point_in_target() {
                        ...
                        if click_count == 3 {
                            ...
                            self.textinput.borrow_mut().select_line_at_index(index);
                            self.is_mouse_selecting.set(false);
                        } else if click_count == 2 {
                            // Double-click: select word at click position
                            self.textinput.borrow_mut().select_word_at_index(index);
                            self.is_mouse_selecting.set(false);
                        } else if shift_held {
        ...
        } else if event.type_() == atom!("mouseup") && !event.DefaultPrevented() {
            ...
            if self.input_type().is_textual_or_password() {
                if let Some(mouse_event) = event.downcast::<MouseEvent>() {
                    ...
                    if let Some(point_in_target) = mouse_event.point_in_target() {
                        ...
                        // (selection_origin will be set if there's a selection)
                        let has_selection = self.textinput.borrow().has_selection() &&
            ...
            self.input_type().is_textual_or_password()
        {
            ...
            if *event.upcast::<Event>().type_() == *"blur" {
                self.owner_document()
                    .embedder_controls()
                    .hide_embedder_control(self.upcast());
                // Stop caret blinking when losing focus
                self.stop_caret_blink_timer();
            }
            if *event.upcast::<Event>().type_() == *"focus" {
                self.handle_focus();
                // Start caret blinking when gaining focus
                self.start_caret_blink_timer();
            }
        }
