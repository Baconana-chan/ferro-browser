...
impl LayoutHTMLTextAreaElementHelpers for LayoutDom<'_, HTMLTextAreaElement> {
    ...
    fn selection_for_layout(self) -> Option<Range<usize>> {
        ...
        let range = UTF8Bytes::unwrap_range(
...
impl HTMLTextAreaElement {
    ...
    /// Start the caret blink timer when the textarea gains focus.
    fn start_caret_blink_timer(&self) {
        ...
        let timer_key = global.run_steps_after_a_timeout(
            ...
            move |global, can_gc| {
                let textarea = trusted.root();
                textarea.handle_caret_blink_tick(global, can_gc);
            },
        );
    ...
    /// Stop the caret blink timer when the textarea loses focus.
    ...
    fn handle_caret_blink_tick(&self, _global: &GlobalScope, _can_gc: CanGc) {
        ...
        // full NodeDamage::Other to avoid repainting the entire textarea on each blink.
