...
impl Painter {
    ...
    pub(crate) fn perform_updates(&mut self) {
        ...
        let mut smooth_scroll_animating = false;
        let scroll_offset_updates: Vec<_> = self
            .webview_renderers
            .values_mut()
            .filter_map(|webview_renderer| {
                // Check if smooth scroll animation is active (Ferro Browser feature).
                if webview_renderer.smooth_scroll_animating() {
                    smooth_scroll_animating = true;
                }
