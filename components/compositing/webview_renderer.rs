...
use crate::smooth_scroll::SmoothScrollState;
...
pub(crate) struct WebViewRenderer {
    ...
    /// Smooth scroll state for momentum/easing scrolling (Ferro Browser feature).
    smooth_scroll_state: SmoothScrollState,
...
impl WebViewRenderer {
    pub(crate) fn new(
        ...
        Self {
            ...
            smooth_scroll_state: SmoothScrollState::new(),
    ...
    /// Returns true if smooth scroll animation is active (Ferro Browser feature).
    pub(crate) fn smooth_scroll_animating(&self) -> bool {
        self.smooth_scroll_state.is_animating()
    }
    ...
    pub(crate) fn process_pending_scroll_and_pinch_zoom_events(
        ...
        // Process smooth scroll animation frame delta (Ferro Browser feature).
        if let Some((delta, point)) = self.smooth_scroll_state.frame_delta() {
            self.pending_scroll_zoom_events
                .push(ScrollZoomEvent::Scroll(ScrollEvent {
